...
class ContrastiveMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, tasks, mask = None):
        ...
        print(self.query_embed.weight[:-1].shape)
        #input()
        ...
        print("Flatten mask features: ", self.class_input_proj(mask_features).shape)
        print("Query embed:", query_embed.shape)
        out_t = out_t[0].permute(1, 0, 2)
        print("Transformer output: ", out_t.shape)
        ...
        print(type(self.forward_prediction_heads))
        ...
        for i in range(self.num_layers):
            print("Number layer:", i)
            level_index = i % self.num_feature_levels
            print("Stage index:", level_index)
            ...
            print("Cross attention head output: ", output.shape)
            output = self.transformer_self_attention_layers[i](
                output, tgt_mask=None,
                tgt_key_padding_mask=None,
                query_pos=query_embed
            )
            print("Self attention head output: ", output.shape)
            # FFN

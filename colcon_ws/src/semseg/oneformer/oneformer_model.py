...
time_inference = []
time_semantic_posprocessing = []
time_panoptic_posprocessing = []
time_instance_posprocessing = []
global batch_number
batch_number = 0
...
class OneFormer(nn.Module):
    ...
    def encode_text(self, text):
        ...
        #print("Output of text encoder: ", x.shape)
        text_x = self.text_projector(x)
        #print("Output of projected text: ", text_x.shape)
        if squeeze_dim:
            ...
            if self.prompt_ctx is not None:
                ...
                #print("Context shape: ", text_ctx.shape)
    ...
    def forward(self, batched_inputs):
        ...
        #print(summary(self.task_mlp, input_data=tasks.float(), col_names=["input_size", "output_size", "num_params"], depth=10))
        tasks = self.task_mlp(tasks.float())
        features = self.backbone(images.tensor)
        #print(summary(self.backbone,input_data=images.tensor, col_names=["input_size", "output_size", "num_params"], depth=10))
        #print("Images shape: ", images.tensor.shape)
        #input()
        ### delete print
        # for i, f in enumerate(features):
        #     print("Feature map: ", i, " shape: ", features[f].shape)
        # print(type(self.sem_seg_head.pixel_decoder))
        #print(summary(self.sem_seg_head.pixel_decoder,input_data=features, col_names=["input_size", "output_size", "num_params"], depth=10))
        ...
        if batch_number % 10 == 0:
            print(batch_number)
        if batch_number  > 200:
            time_inference.append(start.elapsed_time(end))
        if batch_number == 1998:
            print("Inference time mean:", np.mean(time_inference))
            print("Inference time std:", np.std(time_inference))
        ...
        if self.training:
            ...
            # print("Texts encoded shape: ", texts_x["texts"].shape)
            #input()
            ...
            for i, data in enumerate(zip(
                ...
                if self.semantic_on:
                    ...
                    torch.cuda.synchronize()
                    if batch_number > 200:
                        time_semantic_posprocessing.append(start.elapsed_time(end))
                    if batch_number == 1998:
                        print("Semantic postprocessing time mean:", np.mean(time_semantic_posprocessing))
                        print("Semantic postprocessing time std:", np.std(time_semantic_posprocessing))
                ...
                if self.panoptic_on:
                    ...
                    torch.cuda.synchronize()
                    if batch_number > 200:
                        time_panoptic_posprocessing.append(start.elapsed_time(end))
                    if batch_number == 1998:
                        print("Panoptic postprocessing time mean:", np.mean(time_panoptic_posprocessing))
                        print("Panoptic postprocessing time std:", np.std(time_panoptic_posprocessing))                
                ...
                if self.instance_on:
                    ...
                    start.record()         
                    instance_r = self.instance_inference(mask_cls_result, mask_pred_result, input_per_image["task"])
                    ...
                    torch.cuda.synchronize()
                    if batch_number > 200:
                        time_instance_posprocessing.append(start.elapsed_time(end))
                    if batch_number == 1998:
                        print("Instance postprocessing time mean:", np.mean(time_instance_posprocessing))
                        print("Instance postprocessing time std:", np.std(time_instance_posprocessing))     
    ...
    def instance_inference(self, mask_cls, mask_pred, task_type):
        ...
        if self.panoptic_on:
            ...
            scores_per_image = scores_per_image[keep]
            labels_per_image = labels_per_image[keep]
           # print("Mask pred memory:", mask_pred.element_size() * mask_pred.nelement()/ 1024 / 1024)
            mask_pred = mask_pred[keep]
